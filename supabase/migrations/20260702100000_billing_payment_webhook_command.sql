...
begin
  ...
  if p_empresa_id is null or p_subscription_id is null or p_plan_id is null then
    raise exception 'Empresa, assinatura e plano são obrigatórios.'
      using errcode = '22023';
  end if;

  if provider_name = '' or nullif(trim(p_provider_event_id), '') is null
    or nullif(trim(p_provider_payment_id), '') is null then
    raise exception 'Provider, evento e pagamento são obrigatórios.'
      using errcode = '22023';
  end if;
  ...
  if normalized_event_type not in (
    ...
    raise exception 'Tipo de evento de pagamento inválido.' using errcode = '22023';
  end if;
  ...
  if not found then
    raise exception 'Assinatura não encontrada para esta empresa.' using errcode = 'P0002';
  end if;
  ...
  if normalized_event_type = 'PAYMENT_APPROVED' then
    if p_current_period_start is null or p_current_period_end is null then
      raise exception 'Período da assinatura é obrigatório para pagamento aprovado.'
        using errcode = '22023';
    end if;

    if p_current_period_end <= p_current_period_start then
      raise exception 'Período da assinatura inválido.' using errcode = '22023';
    end if;
...
comment on function public.billing_process_payment_webhook(
  ...
  text, timestamptz, timestamptz, timestamptz, timestamptz,
  timestamptz, text, text, jsonb
) is 'Atomic Billing Service boundary for idempotent provider payment webhooks.';
