...
export function TableRow({ children, className, ...props }: HTMLAttributes<HTMLTableRowElement>) {
  return (
    <tr
      className={cn(
        'grid grid-flow-col auto-cols-fr items-center gap-2 rounded-xl border border-slate-200/70 bg-white px-3 py-2.5 shadow-[0_10px_34px_rgb(15_23_42/0.02)] transition duration-200 hover:border-slate-300/80 hover:shadow-[0_16px_48px_rgb(15_23_42/0.035)] sm:gap-3 sm:rounded-[1.4rem] sm:px-5 sm:py-4 sm:hover:-translate-y-0.5 sm:hover:shadow-[0_22px_70px_rgb(15_23_42/0.045)]',
        className,
      )}
      {...props}
    >
...
export function TableCell({ children, className, ...props }: TdHTMLAttributes<HTMLTableCellElement>) {
  return (
    <td
      className={cn(
        'block min-w-0 truncate px-0 py-0 text-xs font-medium text-slate-600 first:text-xs first:font-bold first:text-slate-950 dark:text-slate-600 sm:text-sm sm:first:text-sm sm:first:font-black sm:first:text-base',
        className,
      )}
      {...props}
    >
